...
std::optional<cl_device_id> findDevice(const std::vector<cl_platform_id>& platforms, cl_device_type type) {
    ...
    for (const auto& platform : platforms) {
        const cl_int err = clGetDeviceIDs(platform, type, 1, &device, nullptr);
        if (err == CL_SUCCESS) {
            return device;
        }
...
int main() {
    ...
    cl_device_id device_id = getDevice(CL_DEVICE_TYPE_CPU);
    ...
    std::string kernel_sources;
    {
        ...
        kernel_sources = std::string(std::istreambuf_iterator(file), std::istreambuf_iterator<char>());
